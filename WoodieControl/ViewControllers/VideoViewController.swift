...
class VideoViewController: UIViewController
{
	...
	func start()
	{
        ...
        //NotificationCenter.default.addObserver(self, selector: #selector(orientationDidChange), name: NSNotification.Name.UIDevice.orientationDidChangeNotification, object: nil)
	}
	...
	{
		stopFadeOutTimer()
        UIView.animate(withDuration: FADE_IN_INTERVAL, delay: 0, options: UIView.AnimationOptions.curveEaseIn, animations:
		{
	...
	private func decodeNal(_ nal: [UInt8]) -> Bool
	{
		...
		if let buffer = sampleBuffer, CMSampleBufferGetNumSamples(buffer) > 0
		{
			...
			status = VTDecompressionSessionDecodeFrame(videoSession!, sampleBuffer: buffer, flags: [VTDecodeFrameFlags._EnableAsynchronousDecompression], frameRefcon: nil, infoFlagsOut: &infoFlags)
            //VTDecompressionSessionDecodeFrame(videoSession!, buffer, [VTDecodeFrameFlags._EnableAsynchronousDecompression], nil, &infoFlags)
		}
		return true
	}
